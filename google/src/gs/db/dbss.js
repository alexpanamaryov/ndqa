...
ss.prototype.deleteItem = function( table, id ) {
    return this.manipulate( table, 'update', {state:'inactive'}, {id:id} );

}

ss.prototype.collectSSIds = function() {

    try {
        var SSs = this.getRows('ss',['name','ssid'],{state:'active'});

        for(var i=0; i < SSs.length; i++) {

           /* if(SSs[i].ssid == 'unknown') {*/
                var ssId = SpreadsheetApp.open(DriveApp.getFilesByName(SSs[i].name).next()).getId();

                this.manipulate('ss', 'update', {ssid: ssId, updated: Date.now()}, {name: SSs[i].name});
            /*}*/
        }
    } catch( e ) {
        logError_( e );
...
ss.prototype.collectSIds = function() {

    try {

        var sheets = this.getRows('s',['ss_id','name','sid'],{state:'active'});

        for(var i=0; i < sheets.length; i++) {

            if(sheets[i].sid == 'unknown') {

                var sSheetId = this.getRows('ss',['ssid'],{id: sheets[i].ss_id});

                Logger.log(sSheetId[0].ssid);

                if(sSheetId[0].ssid == 'unknown') {
                    this.collectSSIds();
                }

                sSheetId = this.getRows('ss',['ssid'],{id: sheets[i].ss_id});

                Logger.log(sSheetId[0].ssid);

                var sheet = SpreadsheetApp.openById(sSheetId[0].ssid);

                Logger.log(sheet);

                var sheetId = sheet.getSheetByName(sheets[i].name).getSheetId();

                Logger.log(sheetId);

                this.manipulate('s', 'update', {sid: sheetId, updated: Date.now()}, {name: sheets[i].name});
